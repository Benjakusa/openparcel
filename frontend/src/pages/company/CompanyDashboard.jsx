...
function StatCard({ icon: Icon, label, value, accentColor, sub }) {
    return (
        <div className="glass-card p-6 hover:-translate-y-1 transition-all group">
            ...
            <div className="text-sm sm:text-sm font-normal text-primary">{value}</div>
        ...
        <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
            ...
            {isExpired && (
                <div className="bg-red-50 border border-red-200 rounded-2xl p-4 flex items-center gap-3">
                    ...
                    <div>
                        <div className="font-normal text-red-800 text-sm">Subscription Expired</div>
                        <div className="text-sm text-red-600 font-normal">Your trial has ended.
                            <Link to="/company/subscription" className="ml-1 underline font-normal hover:text-red-500 transition-colors">Buy a plan to continue.</Link>
                        </div>
                    </div>
                </div>
            )}
            {isTrialing && daysLeft <= 5 && (
                <div className="bg-yellow-50 border border-yellow-200 rounded-2xl p-4 flex items-center gap-3">
                    ...
                    <div>
                        <div className="font-normal text-yellow-800 text-sm">Trial ends in {daysLeft} day(s)</div>
                        <div className="text-sm text-yellow-700 font-normal">
                            <Link to="/company/subscription" className="underline font-normal hover:text-yellow-500 transition-colors">Upgrade now</Link> to avoid interruption.
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="glass-card p-5 sm:p-6 border-l-4 border-accent">
                <h1 className="text-base sm:text-base font-semibold text-primary mb-1">{company?.name}</h1>
                <p className="text-gray-500 font-normal text-xs sm:text-sm">
            ...
            {data?.parcels_by_status && (
                <div className="glass-card p-6">
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="text-base font-semibold text-primary">Parcel Status Breakdown</h2>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                        {Object.entries(data.parcels_by_status).map(([status, count]) => (
                            <div key={status} className="bg-gray-50 rounded-2xl p-4 border border-gray-100">
                                <div className={`text-xs font-normal px-3 py-1 rounded-full inline-block mb-3 badge-${status}`}>{status.replace('_', ' ').toUpperCase()}</div>
                                <div className="text-sm sm:text-sm font-normal text-primary">{count}</div>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            {data?.daily_revenue && data.daily_revenue.length > 0 && (
                <div className="glass-card p-0 overflow-hidden">
                    <div className="p-6 border-b border-gray-200/50">
                        <h2 className="text-base font-semibold text-primary">Daily Revenue (Last 30 Days)</h2>
                    </div>
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="bg-gray-50/50 text-left text-xs text-gray-500 uppercase tracking-wider">
                                    ...
                                    <th className="px-6 py-4 font-normal">Cash</th>
                                    <th className="px-6 py-4 font-normal">M-Pesa</th>
                                    <th className="px-6 py-4 font-normal text-primary">Total</th>
                                </tr>
                            </thead>
                                ...
                                {(() => {
                                    ...
                                    return Object.values(grouped).sort((a, b) => b.day.localeCompare(a.day)).map(d => (
                                        <tr key={d.day} className="hover:bg-white/60 transition-colors">
                                            <td className="px-6 py-4 text-gray-700 font-normal">{d.day ? new Date(d.day + 'T00:00:00').toLocaleDateString() : '—'}</td>
                                            <td className="px-6 py-4 text-emerald-600 font-normal">KES {d.cash.toLocaleString()}</td>
                                            <td className="px-6 py-4 text-sky-600 font-normal">KES {d.mpesa.toLocaleString()}</td>
                                            <td className="px-6 py-4 font-normal text-primary">KES {(d.cash + d.mpesa).toLocaleString()}</td>
                                        </tr>
                                    ));
                                })()}
                            </tbody>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
                ...
                <div className="glass-card p-6 border-t-4 border-accent flex flex-col justify-center">
                    <h2 className="text-base font-semibold text-primary mb-2 flex items-center gap-2"><Package size={20} className="text-heading" /> Global Lookup</h2>
                    <p className="text-xs text-gray-500 font-normal mb-4">Directly view the lifecycle details of any parcel bypassing office scope.</p>
                    <form onSubmit={(e) => {
                        e.preventDefault();
                        const val = e.target.elements.tracking.value.trim();
                        if (val) window.location.href = `/parcel/${val}`;
                    }}>
                        <input name="tracking" placeholder="Enter Tracking ID..." className="w-full bg-white/60 border border-gray-200 rounded-xl px-4 py-3 font-mono font-normal text-sm text-primary mb-3 focus:border-accent focus:ring focus:ring-accent/20 transition-all font-normal" />
                        <button type="submit" className="w-full font-normal text-sm py-3 rounded-xl btn-base btn-primary">Track Parcel</button>
                    </form>
                </div>
                ...
                {data?.per_office_performance && (
                    <div className="glass-card p-0 overflow-hidden lg:col-span-2">
                        <div className="p-6 border-b border-gray-200/50">
                            <h2 className="text-base font-semibold text-primary">Office Performance Breakdown</h2>
                        </div>
                        <div className="overflow-x-auto max-h-64">
                            <table className="w-full text-sm">
                                <thead className="sticky top-0 bg-gray-50 z-10 border-b border-gray-200">
                                    <tr className="text-left text-xs text-gray-500 uppercase tracking-wider">
                                        <th className="px-6 py-3 font-normal">Office Name</th>
                                        <th className="px-6 py-3 font-normal">Parcels Dispatched</th>
                                        <th className="px-6 py-3 font-normal text-emerald-600">Successfully Delivered</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-100/50">
                                    {data.per_office_performance.map(o => (
                                        <tr key={o.id} className="hover:bg-white/60 transition-colors">
                                            <td className="px-6 py-4 font-normal text-primary">{o.name}</td>
                                            <td className="px-6 py-4 font-normal text-gray-700">{o.total_dispatched}</td>
                                            <td className="px-6 py-4 font-normal text-emerald-600">{o.successfully_delivered}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
