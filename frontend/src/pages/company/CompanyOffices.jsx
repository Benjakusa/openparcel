...
function OfficeForm({ initial, onSave, onCancel }) {
    ...
    return (
        <div className="fixed inset-0 bg-primary/20 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fadeIn">
            <div className="glass-card bg-white/80 p-8 w-full max-w-md shadow-2xl transition-all">
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
                <div>
                    <h1 className="text-2xl sm:text-3xl font-black text-primary drop-shadow-sm mb-1">Offices</h1>
                    <p className="text-sm font-medium text-gray-500">{offices.length} registered locations</p>
                </div>
                <button onClick={() => { setEditOffice(null); setShowForm(true); }}
                    className="flex items-center gap-2 bg-primary text-white shadow-md hover:shadow-lg hover:-translate-y-0.5 px-5 py-3 rounded-xl text-sm font-black transition-all">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {offices.map(o => (
                    <div key={o.id} className="glass-card bg-white/60 border border-white p-6 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all group flex flex-col justify-between h-full relative overflow-hidden">
                        ...
                        <div className="mb-6 relative z-10">
                            <div className="flex items-start gap-4 mb-4">
                                ...
                                <div>
                                    <h3 className="font-black text-primary text-xl leading-tight group-hover:text-accent transition-colors">{o.name}</h3>
                                </div>
                            </div>
                            ...
                            <div className="space-y-3 pl-1">
                                <div className="flex items-start gap-3 text-sm">
                                    ...
                                    <span className="font-medium text-gray-600">{o.address || 'No address provided'}</span>
                                </div>
                                <div className="flex items-start gap-3 text-sm">
                                    <Phone size={16} className="text-gray-400 mt-0.5 shrink-0" />
                                    <span className="font-medium text-gray-600">{o.phone || 'No phone provided'}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex gap-3 pt-4 border-t border-gray-200/50 relative z-10">
                            <button onClick={() => { setEditOffice(o); setShowForm(true); }}
                                className="flex-1 bg-white hover:bg-gray-50 border border-gray-200 flex items-center justify-center gap-2 text-primary hover:text-accent py-2.5 rounded-xl text-sm font-bold shadow-sm transition-all">
                                <Pencil size={15} /> Edit
                            </button>
                            <button onClick={() => deleteOffice(o.id, o.name)}
                                className="flex-none bg-white hover:bg-red-50 border border-gray-200 hover:border-red-200 flex items-center justify-center text-red-500 w-11 py-2.5 rounded-xl shadow-sm transition-all" title="Delete Office">
                                <Trash2 size={16} />
                            </button>
                        </div>
                    </div>
                ...
                {offices.length === 0 && (
                    <div className="sm:col-span-2 lg:col-span-3 glass-card p-12 text-center border-dashed border-2 border-primary/20 bg-primary/5 cursor-pointer hover:bg-primary/10 transition-colors" onClick={() => { setEditOffice(null); setShowForm(true); }}>
                        ...
                        <h3 className="text-lg font-black text-primary mb-1">No Offices Added</h3>
                        <p className="text-gray-500 font-medium mb-6 text-sm">Start by adding your primary dispatching office</p>
                        <button className="bg-primary text-white font-bold px-6 py-2.5 rounded-xl shadow-sm hover:shadow-md transition-all inline-flex items-center gap-2 text-sm">
