...
import { Package, ScanLine, LayoutDashboard, LogOut, DollarSign, ArrowLeft, Menu, X } from 'lucide-react';
...
export default function OfficeLayout() {
    ...
    const links = [
        { to: '/office', icon: LayoutDashboard, label: 'Home', end: true },
        { to: '/scan', icon: ScanLine, label: 'Scan' },
        { to: '/office/parcels', icon: Package, label: 'Parcels' },
        { to: '/office/revenue', icon: DollarSign, label: 'Revenue' },
    ];
    ...
    return (
        <div className="flex h-screen bg-white relative z-0">
            ...
            ──────────────────────────────────────────────────────────────── */}
            <aside className={` fixed inset-y-0 left-0 flex flex-col z-50 bg-white border-r border-gray-200 transition-transform duration-300 w-64 md:relative md:translate-x-0 md:w-16 md:shadow-none lg:w-64 ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} `}>
                {/* Branding */}
            ...
            <main className="flex-1 overflow-auto flex flex-col bg-white min-w-0">
                ...
                <div className="flex-1 p-4 sm:p-6 w-full max-w-5xl mx-auto">
                    <Outlet />
                </div>
            </main>
        </div>
