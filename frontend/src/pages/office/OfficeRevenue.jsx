    ...
    return (
        <div className="p-4 max-w-2xl mx-auto">
            <div className="bg-primary text-white px-4 py-4 flex items-center gap-3 -mx-4 -mt-4 mb-5">
                ...
                <div>
                    <div className="font-normal">Daily Revenue</div>
            ...
            <div className="grid grid-cols-3 gap-3 mb-5">
                <div className="bg-green-50 rounded-2xl p-4 text-center border border-green-200">
                    ...
                    <div className="text-sm font-normal text-green-800">{totalCash.toLocaleString()}</div>
                ...
                <div className="bg-blue-50 rounded-2xl p-4 text-center border border-blue-200">
                    ...
                    <div className="text-sm font-normal text-blue-800">{totalMpesa.toLocaleString()}</div>
                ...
                <div className="bg-primary/5 rounded-2xl p-4 text-center border border-primary/20">
                    ...
                    <div className="text-sm font-normal text-primary">{(totalCash + totalMpesa).toLocaleString()}</div>
            ...
            <div className="bg-white rounded-2xl border border-pale">
                <div className="px-4 py-3 border-b border-pale/50">
                    <h2 className="font-semibold text-primary text-sm">Revenue by Day</h2>
                </div>
                {loading ? (
                    ...
                    <div className="divide-y divide-pale/50">
                        {days.map(d => (
                            <div key={d.day} className="px-4 py-3 flex items-center justify-between text-sm">
                                <div className="font-normal text-gray-700">{d.day ? new Date(d.day + 'T00:00:00').toLocaleDateString() : '—'}</div>
                                <div className="flex items-center gap-4">
                                    <span className="text-green-700 font-normal">KES {d.cash.toLocaleString()}</span>
                                    <span className="text-blue-700 font-normal">KES {d.mpesa.toLocaleString()}</span>
                                    <span className="font-normal text-primary">KES {(d.cash + d.mpesa).toLocaleString()}</span>
                                </div>
                            </div>
                        ))}
                    </div>
