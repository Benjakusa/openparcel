    ...
    return (
        <div className="p-4 max-w-2xl mx-auto mb-10 mt-6">
            <div className="flex items-center gap-3 mb-8">
                ...
                <div>
                    <h1 className="text-base font-semibold text-primary">Create Parcel</h1>
                    <p className="text-gray-500 font-normal">Payment collected via cash or M-Pesa</p>
                </div>
            </div>
            ...
            {stkPending && (
                <div className="fixed inset-0 bg-primary/20 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="glass-card bg-white/80 p-8 w-full max-w-sm text-center animate-fadeIn">
                        ...
                        <h3 className="font-semibold text-primary text-base mb-2">Waiting for Payment</h3>
                        <p className="text-gray-500 font-normal">STK Push sent to sender's phone. Waiting for M-Pesa PIN entry...</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="glass-card bg-white/40 border border-white/60 p-6 transition-shadow">
                    <h2 className="font-semibold text-primary mb-4 text-sm uppercase tracking-wide flex items-center gap-2">
                    ...
                    <div className="grid sm:grid-cols-2 gap-4">
                        {[['senderName', 'Full Name', true, 'text'], ['senderPhone', 'Phone', true, 'tel'], ['senderIdNumber', 'ID Number', false, 'text']].map(([key, label, req, type]) => (
                            <div key={key} className={key === 'senderName' ? 'sm:col-span-2' : ''}>
                                <label className="block text-xs font-normal text-gray-500 mb-1.5 uppercase tracking-wide">{label}</label>
                ...
                <div className="glass-card bg-white/40 border border-white/60 p-6 transition-shadow">
                    ...
                    <div className="space-y-4">
                        <div className="grid sm:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-xs font-normal text-gray-500 mb-1.5 uppercase tracking-wide">Receiving Office</label>
                                <select required value={form.receivingOfficeId} onChange={e => setForm(f => ({ ...f, receivingOfficeId: e.target.value }))}
                                    className="w-full border border-gray-200/60 bg-white/70 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-accent focus:border-transparent transition-all font-normal">
                                    <option value="" className="text-gray-400">Select destination office...</option>
                                    {offices.map(o => <option key={o.id} value={o.id}>{o.name}</option>)}
                                </select>
                            </div>
                            <div>
                                <label className="block text-xs font-normal text-gray-500 mb-1.5 uppercase tracking-wide">Weight (kg)</label>
                                <input type="number" step="0.1" min="0.1" required value={form.weightKg}
                                    onChange={e => setForm(f => ({ ...f, weightKg: e.target.value }))}
                                    className="w-full border border-gray-200/60 bg-white/70 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-accent focus:border-transparent transition-all font-mono font-normal" />
                            </div>
                        </div>
                        <div>
                            ...
                            <div className="flex gap-3">
                                {['one_time', 'per_kg'].map(type => (
                                    <button key={type} type="button" onClick={() => setForm(f => ({ ...f, parcelType: type, pricingOption: '' }))}
                                        className={`flex-1 py-3 rounded-xl text-sm font-normal border-2 transition-all ${form.parcelType === type ? 'bg-accent text-white border-accent' : 'bg-white/80 text-gray-500 border-white/50 hover:bg-white'}`}>
                        ...
                        {form.parcelType === 'one_time' && options.length > 0 && (
                            <div>
                                <label className="block text-xs font-normal text-gray-500 mb-1.5 uppercase tracking-wide">Pricing Option</label>
                        ...
                        <div>
                            <label className="block text-xs font-normal text-gray-500 mb-1.5 uppercase tracking-wide">Notes (optional)</label>
                ...
                <div className="glass-card bg-white/50 border border-white/60 p-6">
                    ...
                    <div className="flex gap-4">
                        <button type="button" onClick={() => setPaymentMethod('mpesa')}
                            className={`flex-1 py-4 rounded-xl font-normal text-base transition-all border-2 ${paymentMethod === 'mpesa' ? 'bg-primary text-white border-primary scale-[1.02]' : 'bg-white/80 text-gray-500 border-white/50 hover:bg-white'}`}>
                            M-Pesa
                        </button>
                        <button type="button" onClick={() => setPaymentMethod('cash')}
                            className={`flex-1 py-4 rounded-xl font-normal text-base transition-all border-2 ${paymentMethod === 'cash' ? 'bg-emerald-600 text-white border-emerald-600 scale-[1.02]' : 'bg-white/80 text-gray-500 border-white/50 hover:bg-white'}`}>
                            Cash
                        </button>
                    </div>
                </div>
                ...
                {calculatedFee !== null && (
                    <div className="glass-card bg-accent-gradient text-white p-6 flex flex-col sm:flex-row items-center justify-between relative overflow-hidden">
                        ...
                        <div className="mb-2 sm:mb-0 relative z-10 text-center sm:text-left">
                            <div className="text-sm font-normal text-blue-100 uppercase tracking-widest mb-1">Calculated Fee</div>
                            <div className="text-xs font-normal text-blue-200 bg-white/10 px-3 py-1 rounded-full inline-block">{form.parcelType === 'one_time' ? 'Fixed price' : `${form.weightKg || 1} kg × rate`}</div>
                        </div>
                        <div className="text-sm font-normal relative z-10">KES {calculatedFee}</div>
                    </div>
                )}
                {calculatedFee === null && form.receivingOfficeId && (
                    <div className="glass-card bg-amber-50 border border-amber-200 p-4 rounded-2xl text-sm text-amber-800 font-normal text-center">
                        No pricing set for this destination. Contact your company admin.
                    </div>
                )}

                <button type="submit" disabled={submitting}
                    className={`w-full py-5 rounded-2xl font-normal text-sm text-white transition-all hover:-translate-y-0.5 disabled:opacity-60 disabled:hover:translate-y-0 ${paymentMethod === 'cash' ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-primary hover:bg-secondary'}`}>
