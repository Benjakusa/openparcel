        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100/50">
              {loading ? (
                <tr><td colSpan={7} className="py-12 text-center font-bold uppercase tracking-widest text-xs text-gray-400">Loading...</td></tr>
              ) : txs.length === 0 ? (
                <tr><td colSpan={7} className="py-12 text-center font-bold uppercase tracking-widest text-xs text-gray-400">No transactions found</td></tr>
              ) : txs.map((tx, i) => (
                <tr key={`${tx.tx_type}-${tx.id}-${i}`} className="hover:bg-white/60 transition-colors">
                  <td className="px-4 py-3">
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${tx.tx_type === 'subscription' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'}`}>
                      {tx.tx_type}
                    </span>
                  </td>
                  <td className="px-4 py-3 font-semibold text-primary">{tx.company_name || '—'}</td>
                  <td className="px-4 py-3 font-bold">KES {parseFloat(tx.amount || 0).toLocaleString()}</td>
                  <td className="px-4 py-3">
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${TX_STATUS_MAP[tx.status] || 'bg-gray-100 text-gray-600'}`}>{tx.status}</span>
                  </td>
