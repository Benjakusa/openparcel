    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      <div className="glass-card p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-l-4 border-primary">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-semibold text-primary">Platform Activity</h1>
            <p className="text-sm font-normal text-gray-500">Audit trail across all tenants</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button onClick={fetchLogs} className="w-10 h-10 glass-card bg-white/50 border-white/60 flex items-center justify-center rounded-xl btn-base btn-secondary">
            <RefreshCw size={18} />
          </button>
        </div>
      </div>
      ...
      <div className="glass-card p-0 overflow-hidden">
        {loading ? (
          <div className="py-12 text-center font-normal uppercase tracking-widest text-xs text-gray-400">Loading...</div>
        ) : logs.length === 0 ? (
          <div className="py-12 text-center font-normal uppercase tracking-widest text-xs text-gray-400">No activity found</div>
        ) : (
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`text-xs font-normal px-2 py-0.5 rounded-full ${ACTION_COLORS[log.action] || 'bg-gray-100 text-gray-600'}`}>
