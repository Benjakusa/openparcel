      ...
      <div className="space-y-1.5">
        {segments.map((seg, i) => (
          <div key={i} className="flex items-center gap-2 text-xs">
            ...
            <span className="font-bold text-primary">{Math.round((seg.value / total) * 100)}%</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-2">
        ...
        {PERIODS.map(d => (
          <button key={d} onClick={() => setPeriod(d)}
            className={`px-4 py-1.5 rounded-xl text-sm font-bold transition-all ${period === d ? 'bg-accent text-white shadow-sm' : 'bg-white/70 text-gray-600 hover:bg-white border border-gray-200'}`}>
            {d}d
          </button>
        ))}
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="glass-card p-5">
          <h3 className="font-black text-primary text-sm uppercase tracking-wide mb-4 flex items-center gap-2"><DollarSign size={16} /> Daily Platform Revenue</h3>
        ...
        {/* Parcel Volume */}
        <div className="glass-card p-5">
          <h3 className="font-black text-primary text-sm uppercase tracking-wide mb-4 flex items-center gap-2"><Package size={16} /> Daily Parcel Volume</h3>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        {/* New Clients */}
        <div className="glass-card p-5">
          <h3 className="font-black text-primary text-sm uppercase tracking-wide mb-4 flex items-center gap-2"><Building2 size={16} /> New Clients</h3>
          <BarChart data={clientData} valueKey="count" color="#f59e0b" height={140} />
        </div>

        {/* Subscription Breakdown */}
        <div className="glass-card p-5">
          <h3 className="font-black text-primary text-sm uppercase tracking-wide mb-4 flex items-center gap-2"><BarChart3 size={16} /> Subscription Breakdown</h3>
          <DonutChart segments={[
      ...
      {/* Top Clients */}
      <div className="glass-card p-5">
        <h3 className="font-black text-primary text-sm uppercase tracking-wide mb-4 flex items-center gap-2"><DollarSign size={16} /> Top Clients by Parcel Volume</h3>
              ...
              {topClients.length === 0 ? <tr><td colSpan={3} className="py-6 text-center text-gray-400">No data</td></tr> :
                topClients.map((c, i) => (
                  <tr key={c.id} className="border-b border-gray-50">
                    <td className="py-2 text-gray-400 font-bold w-8">{i + 1}</td>
                    <td className="py-2 font-semibold text-primary">{c.name}</td>
                    <td className="py-2 text-right font-medium">{c.parcel_count}</td>
                  </tr>
                ))}
            </tbody>
          </table>
