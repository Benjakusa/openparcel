...
function TrialBadge({ endDate }) {
  ...
  return <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${cls}`}>{days > 0 ? `${days} days` : 'Expired'}</span>;
}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100/50">
              {loading ? (
                <tr><td colSpan={8} className="py-12 text-center font-bold uppercase tracking-widest text-xs text-gray-400">Loading...</td></tr>
              ) : subs.length === 0 ? (
                <tr><td colSpan={8} className="py-12 text-center font-bold uppercase tracking-widest text-xs text-gray-400">No subscriptions found</td></tr>
              ) : subs.map(s => (
                <tr key={s.id} className="hover:bg-white/60 transition-colors">
                  <td className="px-4 py-3 font-black text-primary">{s.name}</td>
                  <td className="px-4 py-3 font-medium text-gray-600 text-xs capitalize">{s.subscription_plan || '—'}</td>
                  <td className="px-4 py-3">
                    <span className={`text-xs font-bold px-2.5 py-1 rounded-full uppercase tracking-wide ${STATUS_MAP[s.subscription_status] || 'bg-gray-100 text-gray-600'}`}>{s.subscription_status}</span>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3 font-bold text-emerald-600">KES {parseFloat(s.total_paid || 0).toLocaleString()}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {s.subscription_status !== 'suspended' && (
                        <button onClick={() => suspend(s.id, s.name)} className="bg-red-50 hover:bg-red-100 border border-red-100 px-2 py-1 rounded-lg text-red-600 text-xs font-bold transition-all">Suspend</button>
                      )}
                    </div>
                  </td>
                </tr>
