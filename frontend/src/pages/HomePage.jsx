...
<nav className="sticky top-0 z-50 glass-card bg-white/80 backdrop-blur-xl border-b border-white">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16 sm:h-20">
        <div className="flex items-center">
            <span className="font-normal text-sm sm:text-sm tracking-tight text-slate-900">
                OpenDesk<span className="text-blue-700">Parcel</span>
            </span>
        </div>
        <div className="flex items-center gap-3 sm:gap-5">
            <Link to="/login" className="text-slate-900 font-normal hover:text-blue-700 transition-colors">
                <span className="hidden sm:inline">Login</span>
                <LogIn size={20} className="sm:hidden" />
            </Link>
            <Link to="/register" className="bg-slate-900 text-white px-4 py-2 sm:px-6 sm:py-2.5 rounded-xl text-xs sm:text-sm font-normal hover:bg-blue-700 transition-all hover:-translate-y-0.5 border border-slate-800">
                Get Started
            </Link>
        </div>
    </div>
                ...
                <section className="pt-24 pb-16 px-4">
                    <div className="max-w-5xl mx-auto glass-card bg-white/60 backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.1)] border border-white p-8 sm:p-16 text-center animate-fadeIn rounded-3xl">
                        <h1 className="text-base md:text-base font-semibold mb-6 leading-tight text-slate-900 drop-shadow-[0_2px_2px_rgba(255,255,255,1)]">
                            Parcel Management<br />
                            <span className="text-blue-700">Built for Kenya</span>
                        </h1>
                        <p className="text-sm sm:text-sm text-slate-800 mb-10 max-w-2xl mx-auto font-normal leading-relaxed drop-shadow-[0_1px_1px_rgba(255,255,255,1)]">
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 justify-center">
                            <Link to="/register" className="bg-blue-700 text-white font-normal px-8 py-4 rounded-2xl hover:bg-blue-800 hover:-translate-y-1 transition-all flex items-center justify-center gap-3 text-sm border border-blue-600">
                                Start Your Trial <ArrowRight size={20} strokeWidth={3} />
                            </Link>
                            <Link to="/login" className="glass-card bg-white/80 border border-white text-slate-900 font-normal px-8 py-4 rounded-2xl hover:bg-white transition-all text-sm text-center hover:-translate-y-1">
                                Sign In
                            </Link>
                        </div>
                    </div>
                ...
                <section className="py-16 px-4">
                    <div className="max-w-6xl mx-auto">
                        <h2 className="text-base lg:text-base font-semibold text-slate-900 text-center mb-4">Everything you need</h2>
                        <p className="text-center text-slate-800 mb-14 font-normal text-sm sm:text-sm">One platform for your entire parcel operations</p>
                        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                            {features.map((f) => (
                                <div key={f.title} className="glass-card bg-white/70 backdrop-blur-lg border border-white shadow-[0_4px_24px_rgba(0,0,0,0.06)] hover:-translate-y-2 transition-all p-6 rounded-3xl group">
                                    ...
                                    <h3 className="font-semibold text-slate-900 mb-3 text-base">{f.title}</h3>
                                    <p className="text-slate-800 font-normal leading-relaxed text-sm">{f.desc}</p>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <section className="py-16 px-4 mb-16">
                    <div className="max-w-5xl mx-auto">
                        <h2 className="text-base lg:text-base font-semibold text-slate-900 text-center mb-4">Simple Pricing</h2>
                        <p className="text-center text-slate-800 mb-14 font-normal text-sm sm:text-sm">Transparent plans to scale your logistics</p>
                        <div className="grid md:grid-cols-2 gap-8">
                            {plans.map((plan) => (
                                <div key={plan.id} className={`glass-card backdrop-blur-xl p-8 relative transition-all duration-300 rounded-3xl ${plan.highlight ? 'bg-blue-50/90 border-2 border-blue-400 shadow-[0_20px_40px_rgba(0,0,0,0.12)] scale-[1.02] md:scale-105' : 'bg-white/70 border border-white hover:scale-[1.02]'}`}>
                                    {plan.highlight && (
                                        <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-blue-700 text-white text-xs font-normal px-5 py-2 rounded-full uppercase tracking-wider">
                                            BEST VALUE
                                        </div>
                                    )}
                                    <h3 className={`text-base font-semibold mb-2 ${plan.highlight ? 'text-blue-800' : 'text-slate-900'}`}>{plan.title}</h3>
                                    <div className="flex items-baseline gap-2 mb-8 pb-6 border-b border-gray-200/60">
                                        <span className="text-sm sm:text-sm font-normal text-slate-900">{plan.price}</span>
                                        <span className="text-sm sm:text-sm text-slate-700 font-normal">{plan.period}</span>
                                    </div>
                                    <ul className="space-y-4 mb-10">
                                        {plan.features.map((f) => (
                                            <li key={f} className="flex items-center gap-3 text-slate-800 font-normal text-sm sm:text-sm">
                                    ...
                                    <Link to="/register" className={`block w-full text-center py-4 rounded-2xl font-normal text-sm transition-all ${plan.highlight ? 'bg-blue-700 text-white hover:bg-blue-800 border border-blue-600' : 'glass-card bg-white/90 text-slate-900 border border-white hover:bg-white hover:text-blue-700'}`}>
                                        Start Your Journey
                                    </Link>
                                </div>
                            ))}
                ...
                {/* Footer */}
                <footer className="mt-auto py-10 bg-white text-center text-sm font-normal text-black w-full z-10 rounded-t-3xl border-t border-gray-200 flex flex-col items-center justify-center gap-3">
                    <span className="font-normal text-sm tracking-tight text-black">
